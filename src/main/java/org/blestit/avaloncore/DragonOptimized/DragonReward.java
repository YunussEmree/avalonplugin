...
import static org.blestit.avaloncore.DragonOptimized.Dragon.SkillCooldown;
...
public class DragonReward implements Listener {
    ...
    public static BossBar dragonBossBar;
    ...
    public void onDragonDamaged(EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof EnderDragon) {
            EnderDragon edragon = (EnderDragon) event.getEntity();
            ...
            if (event.getDamager() instanceof Arrow) {
                ...
                if (dragon != null && !dragon.isDead()) {
                    if (Objects.requireNonNull(edragon.getCustomName()).contains(dragon.getCustomName())) {


                        dragonBossBar.setTitle(dragon.getCustomName() + " - " + Math.round(dragon.getHealth()));
                        dragonBossBar.setProgress(dragon.getHealth() / dragon.getMaxHealth());
                    ...
                    for (Entity entity : entityler) {
                        ...
                        if (!dragonBossBar.getPlayers().contains(p)) {
                            dragonBossBar.addPlayer(p);
                        }
                    }
                    for (Player p : dragonBossBar.getPlayers()) {
                        Entity pEntity = (Entity) p;
                        if (!entityler.contains(pEntity)) dragonBossBar.removePlayer(p);
    ...
    public BossBar bossbar(Location location, Player player, World world){

        //BOSSBAR AYARLARI
        BossBar bar = Bukkit.getServer().createBossBar(dragon.getCustomName() , BarColor.PURPLE, BarStyle.SOLID, BarFlag.PLAY_BOSS_MUSIC);
        bar.setVisible(true);
        bar.setProgress(1);
        ...
        }.runTaskTimer(plugin, 1L, SkillCooldown);
    ...
    public void onkill(EntityDeathEvent event){
        if(event.getEntity() instanceof EnderDragon && dragon != null && dragon.isDead()) {
            if (dragon != null && event.getEntity().getCustomName().contains(dragon.getCustomName())) {

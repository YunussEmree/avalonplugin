...
public class ItemEvents implements Listener {
    ...
    public static HashMap<String, Location> afkmap = new HashMap<String, Location>();

    public static double savascilikseviye;
    GrapplingHook plugin;

    public ItemEvents(GrapplingHook plugin) {
        this.plugin = plugin;
    }
...
//
    ...
    //        double regenhealth = (player.getMaxHealth()/100 + 1.5)/5;
    //        event.setAmount(regenhealth);
    //    }
    //}
    ...
    public void damagefix(EntityDamageEvent event){
        Entity entity = event.getEntity();
        if(entity instanceof Player){
            if(event.getCause() == EntityDamageEvent.DamageCause.FALL || event.getCause() == EntityDamageEvent.DamageCause.DROWNING || event.getCause() == EntityDamageEvent.DamageCause.FIRE || event.getCause() == EntityDamageEvent.DamageCause.LAVA){
